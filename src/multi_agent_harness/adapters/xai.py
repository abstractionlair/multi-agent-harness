"""xAI adapter implementation for the multi-agent harness.

xAI uses an OpenAI-compatible API, so this adapter is similar to OpenAI
but uses XAI_API_KEY and the xAI base URL.
"""
...
class XAIAdapter(ProviderAdapter):
    ...
    def _rest_chat_completions(body: dict[str, Any]) -> dict[str, Any]:
        ...
        if not api_key:
            XAIAdapter._load_dotenv()
            api_key = os.environ.get("XAI_API_KEY")
        if not api_key:
            raise RuntimeError("XAI_API_KEY not set (and .env missing)")
    ...
    def _rest_chat_completions_with_fallback(self, body: dict[str, Any]) -> dict[str, Any]:
        ...
        if last_err:
            raise last_err
        raise RuntimeError("xAI REST chat failed with unknown error")
