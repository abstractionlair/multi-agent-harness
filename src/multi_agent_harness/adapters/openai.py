"""OpenAI adapter implementation for the multi-agent harness.

If the OpenAI SDK is available, uses it; otherwise falls back to REST.
Reads OPENAI_API_KEY from env or .env for REST fallback.
"""
...
class OpenAIAdapter(ProviderAdapter):
    ...
    def _call_with_retries(self, payload: dict[str, Any]) -> Any:
        if self._client is None:
            raise RuntimeError("OpenAI SDK client is not available")
    ...
    def _create_client(self) -> OpenAIType | None:
        ...
        try:
            ...
            return None
        return OpenAIRuntime(api_key=self.api_key)
